    ...
    try {
      ...
      if (error) {
        ...
        const mergedPages = STATIC_PAGES.map((staticPage) => {
          const dbPage = data?.find(
            (d: { page_path: string }) => d.page_path === staticPage.path,
          );
          return {
            id: dbPage?.id,
  ...
  const handleSave = async (page: PageMeta) => {
    ...
    try {
      ...
      if (errorMessage?.includes("does not exist")) {
        toast.error(
          "Database table 'page_metadata' not found. Please create it in Supabase.",
        );
      } else {
          ...
          <CardContent className="pt-6 space-y-4">
            <div className="grid gap-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleChange(page.page_path, "meta_title", e.target.value)
                }
            ...
            <div className="grid gap-2">
              <Label htmlFor={`keywords-${page.page_path}`}>
              ...
              <Input
                ...
                value={page.keywords}
                onChange={(e) =>
                  handleChange(page.page_path, "keywords", e.target.value)
                }
